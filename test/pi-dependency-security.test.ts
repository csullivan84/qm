...
const piCodingAgentTarball = "https://registry.npmjs.org/@earendil-works/pi-coding-agent/-/pi-coding-agent-0.83.0.tgz";
...
test("Pi and MCP security overrides are materialized by the root lockfile", () => {
  ...
  const piPackage = JSON.parse(readFileSync(piManifest, "utf8")) as {
    license?: unknown;
  ...
  const piShrinkwrap = JSON.parse(
    readFileSync(
      new URL("../node_modules/@earendil-works/pi-coding-agent/npm-shrinkwrap.json", import.meta.url),
      "utf8",
    ),
  ) as { lockfileVersion?: unknown };
  assert.equal(typeof piShrinkwrap.lockfileVersion, "number");
  assert.deepEqual(lockedVersions(packages, "brace-expansion"), ["5.0.9"]);
  assert.deepEqual(lockedVersions(packages, "fast-uri").toSorted(), ["3.1.5", "4.1.2"]);
  ...
  assert.equal(installedVersion("hono"), "4.12.34");
  assert.equal(installedVersion("@hono/node-server"), "2.0.10");
  assert.equal(piPackage.license, "MIT");
  assert.equal(piPackage.repository?.url, "git+https://github.com/earendil-works/pi.git");
});
