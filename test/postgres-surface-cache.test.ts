...
test("pg surface-cache: opaque source ids round-trip beside numeric transport timestamps", { skip }, async () => {
  const cache = createPostgresSurfaceCache(URL!);
  try {
    await cache.ingest([
      { container: "CMX", ts: "1785600000.123456", sourceEventId: "$opaque:example.com", text: "hello" },
    ]);
    const messages = await cache.readMessages("CMX");
