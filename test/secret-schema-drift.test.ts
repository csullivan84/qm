...
test("local-auth bootstrap rejects a missing, placeholder, or short setup token", () => {
  const base = { LOCAL_AUTH_BOOTSTRAP_USER: "alice" } as NodeJS.ProcessEnv;
  assert.deepEqual(validateCoreSecretEnv(base), ["LOCAL_AUTH_BOOTSTRAP_TOKEN"]);
  assert.deepEqual(validateCoreSecretEnv({ ...base, LOCAL_AUTH_BOOTSTRAP_TOKEN: "replace-me" }), [
    "LOCAL_AUTH_BOOTSTRAP_TOKEN",
  ]);
  assert.deepEqual(validateCoreSecretEnv({ ...base, LOCAL_AUTH_BOOTSTRAP_TOKEN: "short" }), [
    "LOCAL_AUTH_BOOTSTRAP_TOKEN",
  ]);
  assert.deepEqual(validateCoreSecretEnv({ ...base, LOCAL_AUTH_BOOTSTRAP_TOKEN: "x".repeat(32) }), []);
});
