...
test("local account management refuses agent tokens even for an org admin", async () => {
  const s = start();
  try {
    const response = await fetch(`${s.base}/v1/admin/local-users`, {
      method: "POST",
      headers: {
        "x-agent-capability": await capFor("admin-alice"),
        "content-type": "application/json",
      },
      body: JSON.stringify({ username: "agent-created-user" }),
    });
