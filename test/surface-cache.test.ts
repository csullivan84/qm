...
test("surface cache retains an opaque source event id separately from its sortable timestamp", async () => {
  const cache = createMemorySurfaceCache();
  await cache.ingest([
    { container: "C1", ts: "1785600000.123456", sourceEventId: "$opaque:example.com", text: "hello" },
  ]);
  const messages = await cache.readMessages("C1");
  assert.equal(messages[0]!.sourceEventId, "$opaque:example.com");
});

test("a source-event redaction purges cached body metadata and excludes the message from context", async () => {
  const cache = createMemorySurfaceCache();
  await cache.ingest([
    {
      container: "C1",
      ts: "1.0",
      sourceEventId: "$opaque:example.com",
      text: "sensitive body",
      mentions: { U1: "Alice" },
      files: [{ fileId: "mxc://example.com/sensitive", name: "secret.txt", mimetype: "text/plain" }],
    },
  ]);
  await cache.ingest([
    {
  ...
  assert.deepEqual(await cache.readMessages("C1"), []);
  const [redacted] = await cache.readMessages("C1", { includeDeleted: true });
  assert.equal(redacted?.ts, "1.0");
  assert.equal(redacted?.text, "");
  assert.equal(redacted?.mentions, undefined);
  assert.deepEqual(await cache.search("sensitive"), []);
});
