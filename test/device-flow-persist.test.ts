...
test("a login performed on a shared channel box is keyed to the SCOPE, like its workspace", async () => {
  ...
  assert.equal(res.status, "ok", res.reason ?? "expected ok");
...
test("shared ACMECLI cutover isolates brokered STS without shrinking the existing scopeShared owner union", async () => {
  ...
  assert.equal(owner.status, "ok", owner.reason ?? "expected ok");
  ...
  assert.equal(scoped.status, "ok", scoped.reason ?? "expected ok");
  assert.equal(
    scoped.reply,
  ...
  assert.equal(poisoned.status, "ok", poisoned.reason ?? "expected ok");
  ...
  assert.equal(aliceTurn.status, "ok", aliceTurn.reason ?? "expected ok");
  ...
  assert.equal(aliceAcmecli.status, "ok", aliceAcmecli.reason ?? "expected ok");
  assert.equal(
    aliceAcmecli.reply,
  ...
  assert.equal(retriedDestroy.status, "ok", retriedDestroy.reason ?? "expected ok");
