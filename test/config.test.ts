...
test("local authentication config preserves bootstrap and bounded session lifetimes", () => {
  const configured = loadConfig({
    LOCAL_AUTH_BOOTSTRAP_USER: "alice",
    LOCAL_AUTH_BOOTSTRAP_TOKEN: "fixture-bootstrap-token-that-is-long-enough",
    LOCAL_AUTH_SESSION_TTL_SEC: "3600",
    LOCAL_AUTH_RESET_TTL_SEC: "900",
  });
  assert.equal(configured.localAuthBootstrapUser, "alice");
  assert.equal(configured.localAuthBootstrapToken, "fixture-bootstrap-token-that-is-long-enough");
  assert.equal(configured.localAuthSessionTtlMs, 3_600_000);
  assert.equal(configured.localAuthResetTtlMs, 900_000);
  for (const value of ["0", "299", "300.5", "2592001"]) {
    assert.throws(() => loadConfig({ LOCAL_AUTH_SESSION_TTL_SEC: value }), /LOCAL_AUTH_SESSION_TTL_SEC/);
  }
  for (const value of ["0", "299", "300.5", "604801"]) {
    assert.throws(() => loadConfig({ LOCAL_AUTH_RESET_TTL_SEC: value }), /LOCAL_AUTH_RESET_TTL_SEC/);
  }
});
