...
export async function ensureLocalPostgres(worktree: string, log: (msg: string) => void): Promise<LocalPostgres> {
  const container = process.env.DEV_INSTANCE_POSTGRES_CONTAINER || "qm-dev-postgres18";
  ...
  const volume = process.env.DEV_INSTANCE_POSTGRES_VOLUME || "qm-dev-postgres18-data";
