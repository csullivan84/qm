...
export function createPostgresSurfaceCache(
  ...
  const { q, query, pool, close } = createPgPool(connectionString, [
    `CREATE TABLE IF NOT EXISTS channel_messages(
        org_id TEXT NOT NULL, container TEXT NOT NULL, ts TEXT NOT NULL, source_event_id TEXT,
    ...
    `ALTER TABLE channel_messages ADD COLUMN IF NOT EXISTS source_event_id TEXT`,
    ...
    `CREATE INDEX IF NOT EXISTS channel_messages_by_source_event
        ON channel_messages(org_id, container, source_event_id) WHERE source_event_id IS NOT NULL`,
  ...
  function rowToMessage(r: Record<string, unknown>): CachedMessage {
    return {
      ...
      ts: r.ts as string,
      ...(r.source_event_id != null ? { sourceEventId: r.source_event_id as string } : {}),
    ...
    async ingest(events) {
      ...
      try {
        ...
        for (const e of events) {
          ...
          if (e.deleted && e.sourceEventId) {
            const redacted = await client.query(
              `UPDATE channel_messages
                  SET text = '', mentions = NULL, deleted = TRUE,
                      edited_at = GREATEST(COALESCE(edited_at, 0), $4)
                WHERE org_id = $1 AND container = $2 AND source_event_id = $3
                RETURNING ts`,
              [orgId, e.container, e.sourceEventId, e.createdAt ?? now],
            );
            const timestamps = redacted.rows.map((row) => row.ts as string);
            if (timestamps.length) {
              await client.query(
                `DELETE FROM channel_files WHERE org_id = $1 AND container = $2 AND ts = ANY($3::text[])`,
                [orgId, e.container, timestamps],
              );
            }
            upserted += redacted.rowCount ?? 0;
            continue;
          }
          const res = await client.query(
            `INSERT INTO channel_messages(org_id, container, ts, source_event_id, sub, author_id, author_name, text, mentions, self, bot, mentions_self, edited_at, deleted, handled, created_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12,$13,$14,$15,$16)
             ON CONFLICT (org_id, container, ts) DO UPDATE SET
               source_event_id = COALESCE(EXCLUDED.source_event_id, channel_messages.source_event_id),
  ...
  return {
    async ingest(events) {
      ...
      for (const e of events) {
        ...
        if (e.deleted && e.sourceEventId) {
          const target = [...m.entries()].find(([, message]) => message.sourceEventId === e.sourceEventId);
          if (target) {
            const [targetTs, existingTarget] = target;
            const redacted = {
              ...existingTarget,
              text: "",
              deleted: true,
              editedAt: Math.max(existingTarget.editedAt ?? 0, e.createdAt ?? now),
            };
            delete redacted.mentions;
            m.set(targetTs, redacted);
            const k = key(e.container);
            files.set(
              k,
              (files.get(k) ?? []).filter((file) => file.ts !== targetTs),
            );
            upserted++;
          }
          continue;
        ...
        if (!existing || e.deleted || (e.editedAt ?? 0) >= (existing.editedAt ?? 0)) {
          m.set(e.ts, {
            ...
            ts: e.ts,
            ...((e.sourceEventId ?? existing?.sourceEventId)
              ? { sourceEventId: (e.sourceEventId ?? existing?.sourceEventId) as string }
              : {}),
