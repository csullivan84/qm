...
import { createLocalAuthService, createMemoryLocalAuthPersistence, type LocalAuthService } from "./auth/local-auth.ts";
import { createPostgresLocalAuthPersistence } from "./auth/postgres-local-auth.ts";
...
import { createMatrixInstallationStore, type MatrixInstallationStore } from "./surfaces/matrix-installation.ts";
import {
  createMatrixSyncStateStore,
  type MatrixSyncStateRecord,
  type MatrixSyncStateStore,
} from "./matrix/sync-state.ts";
import { createBackupConfigStore, type StoredBackupConfiguration } from "./backup/config-store.ts";
import { createBackupJobStore, type BackupDeploymentLease, type BackupJobStore } from "./backup/job-store.ts";
import { createRestoreDrillStore } from "./backup/drill-store.ts";
import { createBackupAuditStore } from "./backup/audit-store.ts";
import { createBackupService, type BackupService } from "./backup/service.ts";
import { inspectB2Destination } from "./backup/b2-policy.ts";
import type { BackupAuditEvent, BackupJob, BackupWorkerHeartbeat, RestoreDrill } from "./backup/types.ts";
...
export interface BuiltApp {
  ...
  matrixInstallation: MatrixInstallationStore;
  matrixInstallationDurable: boolean;
  matrixSyncState?: MatrixSyncStateStore;
  backup: BackupService;
  backupManagementDurable: boolean;
...
export function buildApp(
  ...
  const matrixInstallation = createMatrixInstallationStore(
    config.orgId,
    artifactMap("matrix_installation"),
    config.connectorSecretKey ?? randomBytes(32),
    resolve(join(config.dataDir, "matrix-sync-cursor")),
  );
  const matrixSyncState = pgArtifactMap
    ? createMatrixSyncStateStore(artifactMap<MatrixSyncStateRecord>("matrix_sync_state"))
    : undefined;
  const backupConfig = createBackupConfigStore(
    config.orgId,
    artifactMap<StoredBackupConfiguration>("backup_configuration"),
    config.connectorSecretKey ?? randomBytes(32),
  );
  const backupJobs: BackupJobStore = createBackupJobStore(
    artifactMap<BackupJob>("backup_jobs"),
    artifactMap<BackupDeploymentLease>("backup_deployment_leases"),
    artifactMap<BackupWorkerHeartbeat>("backup_worker_heartbeat"),
  );
  const backupDrills = createRestoreDrillStore(artifactMap<RestoreDrill>("backup_restore_drills"));
  const backupAudit = createBackupAuditStore(artifactMap<BackupAuditEvent>("backup_audit_events"));
  const backup = createBackupService({
    ...
    recoveryImage: config.backupRecoveryImage ?? "qm-backup-recovery:development",
    requirePinnedRecoveryImage: config.production,
    inspectDestination: (effective) => inspectB2Destination(effective),
  });
  ...
  const localAuth = createLocalAuthService(
    config.databaseUrl ? createPostgresLocalAuthPersistence(config.databaseUrl) : createMemoryLocalAuthPersistence(),
    {
      ...(config.localAuthSessionTtlMs === undefined ? {} : { sessionTtlMs: config.localAuthSessionTtlMs }),
      ...(config.localAuthResetTtlMs === undefined ? {} : { resetTtlMs: config.localAuthResetTtlMs }),
    },
  );
  const localAuthReady = (async (): Promise<void> => {
    const username = config.localAuthBootstrapUser?.trim();
    const setupCode = config.localAuthBootstrapToken?.trim();
    if (!username) return;
    if (!setupCode) throw new Error("LOCAL_AUTH_BOOTSTRAP_USER requires LOCAL_AUTH_BOOTSTRAP_TOKEN");
    if (!localAuth.durable) throw new Error("local-auth bootstrap requires DATABASE_URL");
    const grants = await admin.listGrants();
    await localAuth.bootstrapAdmin({
      username,
      setupCode,
      adminPrincipals: grants.filter((grant) => grant.role === "org_admin").map((grant) => grant.principalId),
    });
  })();
  void localAuthReady.catch(() => undefined);
  ...
  return {
    ...
    slackInstallation,
    matrixInstallation,
    matrixInstallationDurable: pgArtifactMap !== null,
    ...(matrixSyncState ? { matrixSyncState } : {}),
    backup,
    backupManagementDurable: pgArtifactMap !== null,
    resolveClient,
    consentLinks,
    secretDrops,
    modelGateway,
