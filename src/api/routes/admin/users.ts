...
import { LocalAuthError, normalizeLocalUsername } from "../../../auth/local-auth.ts";
...
export async function listUsers(ctx: ApiCtx): Promise<void> {
  ...
  const [participants, turns, grants, localAccounts] = await Promise.all([
    ...
    deps.localAuth?.listUsers() ?? Promise.resolve([]),
  ]);
  const users = computeUsers({ participants, turns, grants });
  return sendJson(res, 200, { scopeId: scope, users, grants, localAccounts });
}

function localAuthError(ctx: ApiCtx, error: unknown): void {
  if (error instanceof LocalAuthError) {
    sendJson(ctx.res, error.status, { error: "local_user_failed", message: error.message });
    return;
  }
  throw error;
}

function durableLocalAuth(ctx: ApiCtx) {
  const service = ctx.deps.localAuth;
  if (!service?.durable) {
    sendJson(ctx.res, 503, {
      error: "not_configured",
      message: "local user management requires the Postgres-backed account store",
    });
    return null;
  }
  return service;
...
export async function createLocalUser(ctx: ApiCtx): Promise<void> {
  const actor = await authorizeAdmin(ctx, orgScope(ctx.deps));
  if (!actor) return;
  const service = durableLocalAuth(ctx);
  if (!service) return;
  const username = String((ctx.body as { username?: unknown } | undefined)?.username ?? "");
  try {
    const created = await service.createUser(username);
    audit(ctx.deps, {
      principalId: actor.id,
      action: "local_user.create",
      resource: created.user.username,
      scopeLabel: orgScope(ctx.deps),
    });
    return sendJson(ctx.res, 201, { ok: true, ...created });
  } catch (error) {
    return localAuthError(ctx, error);
  }
}

export async function setLocalUserPassword(ctx: ApiCtx): Promise<void> {
  const actor = await authorizeAdmin(ctx, orgScope(ctx.deps));
  if (!actor) return;
  const service = durableLocalAuth(ctx);
  if (!service) return;
  const passwordValue = (ctx.body as { password?: unknown } | undefined)?.password;
  if (typeof passwordValue !== "string") {
    return sendJson(ctx.res, 400, { error: "bad_request", message: "password must be a string" });
  }
  try {
    const username = normalizeLocalUsername(ctx.params.username!);
    await service.setPassword(username, passwordValue);
    audit(ctx.deps, {
      principalId: actor.id,
      action: "local_user.password.set",
      resource: username,
      scopeLabel: orgScope(ctx.deps),
    });
...
export async function resetLocalUserPassword(ctx: ApiCtx): Promise<void> {
  const actor = await authorizeAdmin(ctx, orgScope(ctx.deps));
  if (!actor) return;
  const service = durableLocalAuth(ctx);
  if (!service) return;
  try {
    const username = normalizeLocalUsername(ctx.params.username!);
    const reset = await service.beginPasswordReset(username);
    audit(ctx.deps, {
      principalId: actor.id,
      action: "local_user.password.reset",
      resource: username,
      scopeLabel: orgScope(ctx.deps),
    });
    return sendJson(ctx.res, 200, { ok: true, username, ...reset });
  } catch (error) {
    return localAuthError(ctx, error);
  }
}

export async function setLocalUserEnabled(ctx: ApiCtx): Promise<void> {
  const actor = await authorizeAdmin(ctx, orgScope(ctx.deps));
  if (!actor) return;
  const service = durableLocalAuth(ctx);
  if (!service) return;
  ...
  try {
    const username = normalizeLocalUsername(ctx.params.username!);
    if (!enabled && username === actor.id.trim().toLowerCase()) {
      return sendJson(ctx.res, 400, { error: "bad_request", message: "you cannot disable your own local account" });
    }
    await service.setEnabled(username, enabled);
    audit(ctx.deps, {
      principalId: actor.id,
      action: enabled ? "local_user.enable" : "local_user.disable",
      resource: username,
      scopeLabel: orgScope(ctx.deps),
    });
    return sendJson(ctx.res, 200, { ok: true, enabled });
  } catch (error) {
    return localAuthError(ctx, error);
  }
}
