...
export interface Config {
  ...
  adminGrants?: string;
  localAuthBootstrapUser?: string;
  localAuthBootstrapToken?: string;
  localAuthSessionTtlMs?: number;
  localAuthResetTtlMs?: number;
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  const localAuthSessionTtlSec = numEnvStrict("LOCAL_AUTH_SESSION_TTL_SEC", env.LOCAL_AUTH_SESSION_TTL_SEC);
  const localAuthResetTtlSec = numEnvStrict("LOCAL_AUTH_RESET_TTL_SEC", env.LOCAL_AUTH_RESET_TTL_SEC);
  if (
    localAuthSessionTtlSec !== undefined &&
    (!Number.isSafeInteger(localAuthSessionTtlSec) ||
      localAuthSessionTtlSec < 300 ||
      localAuthSessionTtlSec > 2_592_000)
  ) {
    throw new Error("LOCAL_AUTH_SESSION_TTL_SEC must be an integer from 300 to 2592000");
  }
  if (
    localAuthResetTtlSec !== undefined &&
    (!Number.isSafeInteger(localAuthResetTtlSec) || localAuthResetTtlSec < 300 || localAuthResetTtlSec > 604_800)
  ) {
    throw new Error("LOCAL_AUTH_RESET_TTL_SEC must be an integer from 300 to 604800");
  }
  return {
    ...
    ...(env.LOCAL_AUTH_BOOTSTRAP_USER ? { localAuthBootstrapUser: env.LOCAL_AUTH_BOOTSTRAP_USER } : {}),
    ...(env.LOCAL_AUTH_BOOTSTRAP_TOKEN ? { localAuthBootstrapToken: env.LOCAL_AUTH_BOOTSTRAP_TOKEN } : {}),
    ...(localAuthSessionTtlSec === undefined ? {} : { localAuthSessionTtlMs: localAuthSessionTtlSec * 1000 }),
    ...(localAuthResetTtlSec === undefined ? {} : { localAuthResetTtlMs: localAuthResetTtlSec * 1000 }),
    ...
    ...(env.BACKUP_RECOVERY_IMAGE ? { backupRecoveryImage: env.BACKUP_RECOVERY_IMAGE } : {}),
