...
test("modal background inertness follows every ancestor and restores prior state", () => {
  const dom = new JSDOM(`
    <div id="banner"><button>Sign out</button></div>
    <div id="layout">
      <aside id="sidebar"><button>Files</button></aside>
      <main>
        <div id="page" inert><button>Refresh</button></div>
        <div id="backdrop"><div id="dialog"><button>Cancel</button></div></div>
      </main>
    </div>
  `);
  const document = dom.window.document;
  const modal = document.querySelector<HTMLElement>("#backdrop")!;
...
test("route teardown restores every active modal background and permits later reactivation", () => {
  const dom = new JSDOM('<div id="page"></div><div id="modal"></div>');
  ...
  state = syncModalBackground(state, modal);
  assert.equal(page.inert, true);
  syncModalBackground(state, null);
  assert.equal(page.inert, false);
});
