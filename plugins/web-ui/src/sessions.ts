...
async function commitProjectRename(item: Extract<RecentItem, { kind: "project" }>): Promise<void> {
  ...
  if (await renameProject(project, next)) refreshActiveConversationPresentation();
  renderList();
}
...
function backgroundStatusButton(s: CoreSession): TemplateResult | typeof nothing {
  const background = rowIndicators(s, liveThreads()).background;
  if (!background) return nothing;
  const label = `${background.label} — inspect activity`;
  return html`<button
    class="bg-chip background-status-button"
    type="button"
    title=${label}
    aria-label=${label}
    @click=${(e: Event) => openBackgroundInspector(e, s)}
  >
    ${background.jobs > 0 ? icon(Cog, 11) : nothing}${background.watches > 0 ? icon(Binoculars, 11) : nothing}
  </button>`;
}
...
function sessionRow(s: CoreSession, projectChild = false): TemplateResult {
  ...
  return html`
    <div
      class="session-row ${active ? "active" : ""} ${menuOpen ? "menu-open" : ""} ${readOnly ? "read-only" : ""} ${refreshingTitle ? "title-refreshing" : ""} ${working ? "working" : ""} ${s.awaitingInput ? "awaiting-input" : ""} ${projectChild ? "project-child" : ""} ${saved ? "saved" : ""} ${background ? "has-background" : ""} ${s.color ? "colored" : ""}"
      style=${s.color ? `--session-color:${s.color}` : nothing}
    >
      <button
        class="session"
...
async function refreshSessionTitle(s: CoreSession): Promise<void> {
  ...
  try {
    ...
    if (refreshed.title && !(s.title && s.title.trim())) {
      ...
      refreshActiveConversationPresentation();
    }
    renderList();
  } catch {
    void 0;
...
async function persistSessionPatch(
  ...
  if ("title" in patch) refreshActiveConversationPresentation();
  renderList();
  try {
...
function refreshActiveConversationPresentation(): void {
  for (const conversation of allConversations()) conversation.refreshPresentation();
}
...
export async function openSession(
  ...
  if (historyMode !== "none") cancelPendingRouteRestore();
  if (splitInterceptsOpen(s)) return;
  closeSidebarOnNarrowView();
  if (projectName(s.scopeId) && sessionsState.collapsedProjectScopes.delete(s.scopeId)) renderList();
  return openSessionInto(mainConversation(), s, entriesPrefetch, historyMode, announceRoute);
}
...
export async function openSessionInto(
  ...
  if (continuable) {
    ...
    conv.mountContinuable(s.threadRef, s.id, s.scopeId, messages, s.channelName ?? null, historyMode, announceRoute);
    conv.setTranscriptWindow(anchorSeq, earlier);
  } else {
    conv.mountReadOnly(s, messages, earlier, anchorSeq, historyMode, announceRoute);
  }
  renderList();
}
