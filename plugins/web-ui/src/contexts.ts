...
export async function renameProject(project: CoreProject, name: string): Promise<boolean> {
  try {
    ...
    if (appState.currentView === "contexts") {
      drawContexts();
      updateNavigationPresentation();
    }
    return true;
...
export function selectContext(scopeId: string | null, historyMode: "push" | "replace" | "none" = "push"): void {
  memberSearchSeq++;
  cancelMemberSearchTimer();
  contextsState.memberProjectId = null;
  contextsState.memberQuery = "";
  ...
  drawContexts();
  updateNavigationPresentation(historyMode !== "replace", historyMode !== "replace");
  if (scopeId) {
