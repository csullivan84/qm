...
let cronModalBackground: ModalBackgroundState | null = null;
let activeCronId: string | null = null;
let pendingCronId: string | null = null;

export function resetActiveCron(): void {
  cronModalBackground = syncModalBackground(cronModalBackground, null);
  cronsScope = null;
  cronDialog = null;
  cronDialogFocus = null;
}
...
export async function renderCronsPage(): Promise<void> {
  ...
  if (cronList.length === 0 && visibleCronList.length === 0) cronsLoading = true;
  drawCronsPage();
  await ensureContexts();
  if (seq !== appState.viewRenderSeq || appState.currentView !== "crons") return;
  drawCronsPage();
  ...
  return html`
    <div class="cron-row-actions" aria-label="Cron actions">
      <button
        ...
        data-cron-dialog-kind="rename"
        data-cron-id=${c.id}
        @click=${(event: Event) => showCronDialog("rename", c, event.currentTarget as HTMLElement)}
      >
        ${icon(Pencil, 14)}
      </button>
      ${stateAction}
...
function openCron(c: CronView, opts: { push?: boolean } = {}): void {
          ...
          manageable
            ? html`
                <div class="actions">
                  ...
                  <button
                    class="btn danger"
                    data-cron-dialog-kind="delete"
                    data-cron-id=${c.id}
                    @click=${(event: Event) => showCronDialog("delete", c, event.currentTarget as HTMLElement)}
                  >
  ...
  if (cronDialog?.cron.id === c.id) setCronDialogBackgroundInert(true);
...
function showCronDialog(kind: "rename" | "delete", cron: CronView, opener: HTMLElement): void {
  cronDialogFocus = { opener, cronId: cron.id, kind, returnToList: Boolean(opener.closest(".cron-row")) };
  cronDialog = { kind, cron };
  openCron(cron);
  requestAnimationFrame(() => {
    if (cronDialog?.kind !== kind || cronDialog.cron.id !== cron.id) return;
    if (kind === "rename") document.querySelector<HTMLInputElement>(".cron-edit-dialog input")?.focus();
    else focusDialogCancel(document);
  });
}

function closeCronDialog(c: CronView): void {
  const focus = cronDialogFocus;
  setCronDialogBackgroundInert(false);
  cronDialog = null;
  cronDialogFocus = null;
  if (focus?.returnToList) drawCronsPage();
  else openCron(c);
  restoreCronDialogFocus(focus);
}

function setCronDialogBackgroundInert(inert: boolean): void {
  const backdrop = inert ? (appState.mainEl?.querySelector<HTMLElement>(".project-dialog-backdrop") ?? null) : null;
  cronModalBackground = syncModalBackground(cronModalBackground, backdrop);
}

function restoreCronDialogFocus(focus: typeof cronDialogFocus): void {
  requestAnimationFrame(() => {
    if (!focus || cronDialog) return;
    restoreDialogFocus(
      focus.opener,
      () =>
        [...document.querySelectorAll<HTMLElement>("[data-cron-dialog-kind][data-cron-id]")].find(
          (element) => element.dataset.cronId === focus.cronId && element.dataset.cronDialogKind === focus.kind,
        ) ?? appState.mainEl,
    );
  });
}
  ...
  if (dialog.kind === "delete") {
    ...
    >
      <div
        ...
        @keydown=${(event: KeyboardEvent) => trapDialogFocus(event, () => closeCronDialog(c))}
      >
        <div class="project-dialog-head">
          <div><h2 id="cron-delete-title">Delete ${cronTitle(c)}?</h2></div>
        </div>
        <p id="cron-delete-description">
          This permanently removes the schedule and its retained run history. Archive it instead if you may need it
          later.
        </p>
        <div class="project-dialog-actions">
          <button class="btn" type="button" data-dialog-cancel @click=${() => closeCronDialog(c)}>Cancel</button>
...
async function saveCronEdit(event: SubmitEvent, c: CronView): Promise<void> {
  ...
  if (!ok) {
    requestAnimationFrame(() => document.querySelector<HTMLInputElement>(".cron-edit-dialog input")?.focus());
    return;
  }
  const focus = cronDialogFocus;
  setCronDialogBackgroundInert(false);
  ...
  await refreshCrons();
  const updated = cronList.find((item) => item.id === c.id) ?? visibleCronList.find((item) => item.id === c.id);
  if (focus?.returnToList || !updated) drawCronsPage();
  else openCron(updated);
  restoreCronDialogFocus(focus);
}
...
async function confirmDeleteCron(id: string): Promise<void> {
  await cronMutate(async () => {
    ...
    await refreshCrons();
    const current = cronList.find((item) => item.id === id) ?? visibleCronList.find((item) => item.id === id);
    if (focus?.returnToList || !current) drawCronsPage();
    else openCron(current);
    restoreCronDialogFocus(focus);
  }, undefined);
}
