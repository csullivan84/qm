...
import { restoreDialogFocus, syncModalBackground, trapDialogFocus, type ModalBackgroundState } from "./dialog-focus";
...
export function createComposerSurface(ctx: ConvCtx): ComposerSurface {
  ...
  let pasteDialogOpener: HTMLElement | null = null;
  let pasteModalBackground: ModalBackgroundState | null = null;
  ...
  function resetComposer(): void {
    pasteModalBackground = syncModalBackground(pasteModalBackground, null);
    composerState.draft = "";
                          ...
                          pastedTextIds.has(a.id)
                            ? html`
                                <button
                                  ...
                                  @click=${(event: Event) =>
                                    openPasteView(a.id, agent, event.currentTarget as HTMLElement)}
                                >
                                  ${icon(FileText, 14)}
          ...
          approvalPauses.length
            ...
            : html`
                <textarea
                  ...
                  aria-controls=${slash.open ? "slash-command-listbox" : nothing}
                  aria-activedescendant=${
                    slash.open && slash.matches.length > 0
                      ? slashOptionId(clampedActive(slash.matches.length))
                      : nothing
                  }
                  aria-describedby="slash-command-status"
                  rows="1"
                ...
                ></textarea>
                <span class="sr-only" id="slash-command-status" role="status" aria-live="polite" aria-atomic="true"
                  >${slashStatus(slash)}</span
                >
              `
        }
  ...
  function pasteViewDialog(agent: Agent): TemplateResult | typeof nothing {
    ...
    return html`
      ...
      >
        <div
          class="project-dialog paste-dialog"
          role="dialog"
          aria-modal="true"
          aria-labelledby="paste-dialog-title"
          @keydown=${(event: KeyboardEvent) => trapDialogFocus(event, () => closePasteView(agent))}
        >
          ...
          <label class="paste-dialog-label" for="paste-dialog-text">Pasted text content</label>
          <textarea
            id="paste-dialog-text"
  ...
  function openPasteView(id: string, agent: Agent, opener: HTMLElement): void {
  ...
  function closePasteView(agent: Agent): void {
    ...
    requestAnimationFrame(() =>
      restoreDialogFocus(
        opener,
        () =>
          ctx.chat.state.host?.querySelector<HTMLElement>(`[data-paste-id="${CSS.escape(pasteId)}"]`) ??
          ctx.chat.state.host?.querySelector<HTMLElement>(".composer-input"),
      ),
    );
  }
  ...
  function syncPasteDialogAccessibility(): void {
    const shell = ctx.chat.state.host?.querySelector<HTMLElement>(".custom-chat-shell");
    const backdrop = composerState.pasteView
      ? (shell?.querySelector<HTMLElement>(".project-dialog-backdrop") ?? null)
      : null;
    pasteModalBackground = syncModalBackground(pasteModalBackground, backdrop);
  }
  ...
  function slashMenu(agent: Agent): TemplateResult | typeof nothing {
    ...
    if (slash.loading && slash.matches.length === 0) {
      return html`<div class="slash-popover" id="slash-command-listbox" role="listbox" aria-label="Skills">
    ...
    return html`
      <div class="slash-popover" id="slash-command-listbox" role="listbox" aria-label="Skills">
        <div class="menu-title">Skills</div>
        ${slash.matches.map((m, i) => slashRow(m, i, i === active, agent))}
      </div>
    `;
  }

  function slashOptionId(index: number): string {
    return `slash-command-option-${index}`;
  }

  function slashStatus(slash: ReturnType<typeof currentSlashMenu>): string {
    if (slashQuery(composerState.draft) === null || composerState.slashDismissed) return "";
    if (slash.loading && slash.matches.length === 0) return "Loading skills.";
    if (slash.matches.length === 0) return "No matching skills.";
    return `${slash.matches.length} matching ${slash.matches.length === 1 ? "skill" : "skills"}.`;
  }

  function slashRow(m: SkillMatch, index: number, active: boolean, agent: Agent): TemplateResult {
    return html`
      <div
  ...
  function removeAttachment(id: string, agent: Agent): void {
    const closingPaste = composerState.pasteView?.id === id;
    const opener = closingPaste ? pasteDialogOpener : null;
    ...
    if (closingPaste)
      requestAnimationFrame(() =>
        restoreDialogFocus(opener, () => ctx.chat.state.host?.querySelector<HTMLElement>(".composer-input")),
      );
  }
