...
import { formatAssistantCompletionAnnouncement } from "./assistant-announcement";
import {
  adminSessionLogUrl,
  appState,
  can,
...
import type { NavigationHistoryMode } from "./navigation-history";
import {
  addPendingSession,
  groupDmTitle,
  refreshSessions,
...
export function createChatSurface(ctx: ConvCtx): ChatSurface {
  ...
  let assistantCompletionAnnouncement = "";
  ...
  function teardownActiveChat(): void {
    ...
    assistantCompletionAnnouncement = "";
  }
  ...
  function newChat(
    context?: { scopeId: string; name: string | null },
  ...
  function mountContinuable(
    ...
    const agent = new Agent({
      ...
      streamFn: () => {
        throw new Error("Stream function unavailable.");
      },
    });
    ...
    agent.subscribe((e) => {
      ...
      if (agent === chatState.agent && e.type === "agent_end") announceCompletedAssistantReply(agent);
      if (agent === chatState.agent && (agent.state.isStreaming || e.type === "agent_end"))
    ...
    const focusComposer = focusComposerOnMount || !announceRoute;
    if (ctx.ownsUrl) updateNavigationPresentation(announceRoute, announceRoute && !focusComposer);
    if (focusComposer) ctx.composer.focusComposerEnd();
  ...
  function syncLocation(historyMode: NavigationHistoryMode = "replace"): void {
    if (ctx.ownsUrl) syncUrlFromState(historyMode);
  ...
  async function approveCommand(agent: Agent, decision: ApprovalDecision): Promise<void> {
    ...
    const previousReplyKey = assistantReplyKey(agent);
    ...
    try {
      ...
      if (agent === chatState.agent) {
        ...
        const refreshed = await refreshTranscriptFromEntries(agent);
        if (refreshed && agent === chatState.agent) {
          announceCompletedAssistantReply(agent, previousReplyKey);
          drawActiveChat(agent);
        }
      }
    }
  ...
  async function refreshTranscriptFromEntries(agent: Agent): Promise<boolean> {
    const sessionId = chatState.sessionId;
    if (!sessionId || agent !== chatState.agent || agent.state.isStreaming) {
      drawActiveChat(agent);
      return false;
    }
  ...
  async function resumeTrackedRun(
    ...
    const seedText = popped
      ...
      .filter(Boolean)
      .join("\n\n");
    agent.streamFunction = makeRunResumeStreamFn(activeRun.runId, activeRun.run, onWork, runSlot, seedText);
    try {
      await agent.continue();
    } catch (err) {
      if (agent === chatState.agent)
  ...
  function mountLoadingPane(): void {
    ...
    render(
      html`<div class="custom-chat-shell">
        <h1 class="sr-only" tabindex="-1">Loading conversation</h1>
  ...
  function mountReadOnly(
    ...
    const draw = () => {
      const session = readOnlyView?.id === s.id ? readOnlyView.session : s;
      render(
        html`
          <div class="custom-chat-shell">
            ${chatHeader(groupDmTitle(session), surfaceOf(session), true)}
            ${newestReplyControl(messages.some((message) => assistantRenderState(message, false)?.visible === true))}
            <div class="readonly-banner">
              ${
                surfaceOf(session) === "slack"
                  ? html`This conversation lives in Slack. Replies happen
                    there.${
                      sessionSlackUrl(session)
                        ? html` <a
                            class="readonly-banner-link"
                            href=${sessionSlackUrl(session)!}
                            target="_blank"
                            rel="noreferrer"
                            >Open in Slack</a
                          >`
            ...
            <section class="chat-scroll readonly-scroll" aria-label="Conversation transcript">
              <div class="message-stack" aria-live="off" aria-busy="false">
                ${
                  earlierCount > 0
                        ...
                        <button
                          ...
                          @click=${async (e: Event) => {
                            ...
                            try {
                              ...
                              if (chatState.sessionId !== session.id) return;
    ...
    draw();
    container.replaceChildren(host);
    if (ctx.ownsUrl) updateNavigationPresentation(announceRoute, announceRoute);
    ctx.ensureDeliveryStream();
    consumeBackgroundPanelRequest();
  }
  ...
  function drawActiveChat(agent = chatState.agent, opts: { forceScroll?: boolean } = {}): void {
    ...
    const hasCompletedReply = messages.some(
      (message) =>
        !(agent.state.isStreaming && message === agent.state.streamingMessage) &&
        assistantRenderState(message, false)?.visible === true,
    );
    render(
      html`
        <div
        ...
        >
          <h1 class="sr-only conversation-title" tabindex="-1">${activeConversationTitle()}</h1>
          ${newestReplyControl(!glanceTier && hasCompletedReply)}
          <div class="sr-only" id="assistant-reply-status" role="status" aria-live="polite" aria-atomic="true">
            ${assistantCompletionAnnouncement}
          </div>
          ${
          ...
          ${
            glanceTier
              ...
              : html`<section class="chat-scroll" aria-label="Conversation transcript" @scroll=${onTranscriptScroll}>
                  <div
                    class="message-stack ${messages.length ? "" : "empty-stack"}"
                    aria-live="off"
                    aria-busy=${agent.state.isStreaming ? "true" : "false"}
                  >
    ...
    ctx.composer.syncPasteDialogAccessibility();
    if (ctx.ownsUrl) updateNavigationPresentation();
    ctx.composer.resizeComposer();
    scrollTranscript(opts.forceScroll);
    postCurrentPaneState();
  }

  function newestReplyControl(show: boolean): TemplateResult | typeof nothing {
    if (!show) return nothing;
    return html`<button class="a11y-jump-latest" type="button" @click=${focusNewestAssistantReply}>
      Jump to newest assistant reply
    </button>`;
  }

  function focusNewestAssistantReply(): void {
    const replies = chatState.host?.querySelectorAll<HTMLElement>(".assistant-row:not(.streaming)");
    ...
    focus();
    requestAnimationFrame(() => {
      if (document.activeElement !== newest) focus();
    });
  }

  function latestAssistantReply(agent: Agent): AssistantMessage | undefined {
    return [...visibleMessages(agent)]
      .reverse()
      .find((message) => (message as { role?: string }).role === "assistant") as AssistantMessage | undefined;
  }

  function assistantReplyKey(agent: Agent): string | null {
    const reply = latestAssistantReply(agent);
    if (!reply) return null;
    const work = (reply as AssistantWork).work;
    return JSON.stringify([
      messageText(reply),
      reply.stopReason ?? "",
      reply.errorMessage ?? "",
      work?.status ?? "",
      (reply as AssistantWork & { approvalDecision?: string }).approvalDecision ?? "",
    ]);
  }

  function assistantCompletionKey(agent: Agent): string | null {
    const reply = latestAssistantReply(agent);
    if (!reply || (reply as AssistantWork).work?.pendingApprovals?.length) return null;
    return assistantReplyKey(agent);
  }

  function announceCompletedAssistantReply(agent: Agent, previousKey: string | null = null): void {
    const reply = latestAssistantReply(agent);
    const completionKey = assistantCompletionKey(agent);
    if (!reply || !completionKey || completionKey === previousKey) {
      assistantCompletionAnnouncement = "";
      return;
    }
    assistantCompletionAnnouncement = formatAssistantCompletionAnnouncement(
      messageText(reply),
      reply.stopReason,
      reply.errorMessage,
    );
  ...
  function activeConversationTitle(): string {
    if (ctx.ownsUrl) return navigationLabel();
    ...
    return chatState.contextName ? `New chat in ${chatState.contextName}` : "New chat";
  }

  function refreshActiveConversationPresentation(): void {
    if (appState.currentView !== "chats") return;
    if (readOnlyView) {
      const updated = sessionsState.list.find((session) => session.id === readOnlyView?.id);
      if (updated) readOnlyView.session = updated;
      readonlyRedraw?.();
      if (ctx.ownsUrl) updateNavigationPresentation();
      return;
    }
    if (!chatState.host) return;
    const label = activeConversationTitle();
    const heading = chatState.host.querySelector<HTMLHeadingElement>("h1");
    if (heading && heading.textContent?.trim() !== label) {
      if (chatState.agent) {
    ...
    return html`
      <header class="chat-topbar">
        <div class="chat-heading">
          <h1 class="chat-title" tabindex="-1">${title}</h1>
  ...
  function chatMessage(message: AgentMessage, index: number, isStreaming = false): TemplateResult | typeof nothing {
    ...
    if (role === "user" || role === "user-with-attachments") {
      ...
      return html`
        <article
          ...
          data-index=${index}
          aria-labelledby=${`message-${index}-speaker`}
        >
          <span class="sr-only" id=${`message-${index}-speaker`}>You</span>
    ...
    if (role === "assistant") {
      const state = assistantRenderState(message, isStreaming);
      if (!state?.visible) return nothing;
      const { msg, work, showWork, deliveredFiles } = state;
      return html`
        <article
        ...
        >
          <span class="sr-only" id=${`message-${index}-speaker`}>Assistant</span>
          <div class="assistant-body">
            ${showWork && work ? workBlock(work, isStreaming) : nothing} ${assistantContent(msg, isStreaming, showWork)}
  ...
  function assistantRenderState(message: AgentMessage, isStreaming: boolean) {
    if ((message as { role?: string }).role !== "assistant") return null;
    ...
    return { msg, work, showWork, deliveredFiles, visible };
  }
  ...
  return {
    ...
    refreshPresentation: refreshActiveConversationPresentation,
    dispose,
  };
}
