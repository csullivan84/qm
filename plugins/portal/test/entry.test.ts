...
test("production local auth boots without OIDC and refuses incompatible modes", () => {
  const command = "import('./src/index.ts').then(m => m.bootChecks())";
  const baseEnv: NodeJS.ProcessEnv = {
    ...process.env,
    NODE_ENV: "production",
  ...
  for (const incompatible of [{ AUTH_BROKER_UPSTREAM: "http://auth.internal:8080" }, { PORTAL_PLAYGROUND: "1" }]) {
    const refused = spawnSync(process.execPath, ["--input-type=module", "-e", command], {
      cwd: process.cwd(),
      env: { ...baseEnv, ...incompatible },
      encoding: "utf8",
    });
    assert.notEqual(refused.status, 0);
    assert.match(refused.stderr, /PORTAL_AUTH_MODE=local cannot be combined/);
  }
});
