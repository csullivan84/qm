# Portal — the public authenticated front door
...
fronts every surface. It supports Postgres-backed **local username/password accounts** and
provider-neutral **OIDC** sign-in (Google Workspace is the email-first OIDC deployment default),
then reverse-proxies — over Fly's private 6PN — to the
...
With the default `PORTAL_AUTH_MODE=oidc`:
...
With `PORTAL_AUTH_MODE=local`, the portal posts the submitted username/password to a
source-authenticated core endpoint. The core verifies a salted scrypt password hash and creates an
opaque, hashed, Postgres-backed session. The browser receives only the opaque token in the same
HttpOnly `portal_session` cookie. Every request validates that token against the core, so logout,
password changes, resets, and account disabling revoke active access immediately.

In either mode:

1. **Proxy** — every other path requires a valid session. The portal picks the upstream by the
...
- **Identity comes only from the configured authentication authority.** In OIDC mode that is the
  verified provider subject; in local mode it is the username attached to a live core session. The
  browser never asserts it. The upstream request is built **from scratch** — an _allowlist_ of safe headers
...
- **OIDC non-admin users need no per-id config.** `PORTAL_EXPECTED_TEAM_ID` pins the workspace, so any
...
- **Secret hygiene.** In production the portal refuses to boot unless `PORTAL_SESSION_SECRET` and
  `PORTAL_IDENTITY_SECRET` are set and distinct from core ingress auth. OIDC mode also requires
  `OIDC_CLIENT_SECRET`.
...
- **OIDC stateless logout.** In OIDC mode, `POST /auth/logout` clears the cookie but can't revoke an
  already-issued session before `exp`; the core's `canAdminister` (re-read per request) remains the live admin
  revocation path. Slack has no RP-initiated end-session, so SSO re-login is silent.

## Local database accounts

Set `PORTAL_AUTH_MODE=local` on the portal and use a Postgres `DATABASE_URL` on the core. Local mode
does not use any OIDC variables or the built-in email broker. Passwords are accepted only by the
portal and source-authenticated core routes, stored only as salted scrypt hashes, and never returned
by APIs. Session and one-time setup tokens are random opaque values whose hashes are the only values
stored in Postgres.

For the initial administrator, set `LOCAL_AUTH_BOOTSTRAP_USER` on the core to a principal that
already has an `org_admin` grant in `ADMIN_GRANTS`, and inject a generated
`LOCAL_AUTH_BOOTSTRAP_TOKEN` of at least 32 characters. Open `/auth/setup`, enter that one-time code,
and choose the account password. Once setup succeeds, remove both bootstrap variables; subsequent
starts never replace an existing password or administrator-issued setup code. Fly may retain the
staged token after its non-secret user setting is removed; a token without a configured bootstrap
user is inert.

An org admin manages accounts under **Admin → Users → Local sign-in accounts**. Creating an account
or resetting a password returns a one-time setup code to deliver out of band. Setting a password
directly is also available when the administrator and user are sharing it through an appropriate
secure channel. Issuing a reset code leaves the existing password and sessions active until the code
is used, at which point the password changes and every prior session is revoked. Direct password
changes and disabling an account also revoke active sessions. Passwords are never included in
responses or audit records.

`LOCAL_AUTH_SESSION_TTL_SEC` configures core session lifetime from 300 seconds through 30 days; the
default is 8 hours. `LOCAL_AUTH_RESET_TTL_SEC` configures ordinary setup-code lifetime from 300
seconds through 7 days; the default is 1 hour. Bootstrap setup codes last 7 days. Portal login and
setup attempts use the core's durable claim store for rate limiting; tune
`PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_USER`, `PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_IP`, and
`PORTAL_LOCAL_AUTH_LOGIN_WINDOW_S` only when the defaults of 10 per username, 30 per address, and 15
minutes are unsuitable.
...
Non-secret (`[env]`): `PORT` (8097 local / 8080 image), `PORTAL_PUBLIC_URL`, `PORTAL_AUTH_MODE`,
`CORE_API_URL`, `CORE_ORG_ID`, `WEB_UI_UPSTREAM`, `ADMIN_UPSTREAM`,
