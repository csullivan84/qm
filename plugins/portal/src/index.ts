...
import { json, escapeHtml, readBody, serveEmojiFavicon, PayloadTooLargeError } from "../../chassis/src/http.ts";
import {
...
const AUTH_MODE = process.env.PORTAL_AUTH_MODE?.trim() || "oidc";
...
const LOCAL_AUTH_LOGIN_LIMIT_PER_USER = Number(process.env.PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_USER ?? 10);
const LOCAL_AUTH_LOGIN_LIMIT_PER_IP = Number(process.env.PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_IP ?? 30);
const LOCAL_AUTH_LOGIN_WINDOW_S = Number(process.env.PORTAL_LOCAL_AUTH_LOGIN_WINDOW_S ?? 900);
...
async function coreLocalAuth(
  path: "/login" | "/session" | "/logout" | "/setup",
  payload: Record<string, unknown>,
): Promise<{ status: number; body: Record<string, unknown> }> {
  const corePath = withSourceAuthNonce(`/v1/auth/local${path}`, CORE_SIGNING_SECRET);
  const body = JSON.stringify(payload);
  const response = await fetch(`${CORE}${corePath}`, {
    method: "POST",
    headers: { ...signedHeaders(CORE_SIGNING_SECRET, "POST", corePath, body), "content-type": "application/json" },
    body,
...
async function localDatabaseSession(req: IncomingMessage): Promise<SessionClaims | null> {
  const token = readCookie(req.headers.cookie, "portal_session");
  if (!token) return null;
  try {
    const result = await coreLocalAuth("/session", { token });
    const username = result.body.username;
    const expiresAt = result.body.expiresAt;
    if (result.status !== 200 || typeof username !== "string" || typeof expiresAt !== "number") return null;
    const now = Math.floor(Date.now() / 1000);
    return { k: "session", sub: username, org: ORG, auth: now, iat: now, exp: Math.floor(expiresAt / 1000) };
  } catch {
    return null;
  }
}
...
async function currentSession(req: IncomingMessage): Promise<SessionClaims | null> {
  if (AUTH_MODE === "local") return localDatabaseSession(req);
  return (
...
const CARD_STYLE = `<style>
  :root{
    ...
    --border:#e5e5e5; --secondary:#f5f5f5; --warn:#b42318; --warn-bg:#fdeceb; --focus:#005fcc;
  ...
  @media (prefers-color-scheme:dark){
    :root{ --bg:#0a0a0a; --surface:#171717; --text:#fafafa; --muted:#a3a3a3;
      --border:#2a2a2a; --secondary:#262626; --warn:#ff8a80; --warn-bg:#2a1a1a; --focus:#8ec5ff;
  ...
  *{ box-sizing:border-box; }
  ...
  .auth-form label{ color:var(--text); font-size:13px; font-weight:600; }
  .field-help{ color:var(--muted); font-size:12.5px; margin:-4px 0 0; }
  .auth-form input{ width:100%; min-height:44px; padding:9px 11px; color:var(--text);
    background:var(--bg); border:1px solid var(--border); border-radius:var(--radius-md); font:inherit; }
  .auth-form input:focus{ outline:2px solid var(--focus); outline-offset:1px; }
  .auth-error{ color:var(--warn); background:var(--warn-bg); border:1px solid var(--border);
    border-radius:var(--radius-md); padding:10px 12px; font-size:13px; text-align:left; }
...
<body>
  <main>
    <section class="card${o.wide ? " wide" : ""}" aria-labelledby="t">
      ...
      <p class="msg" id="page-description">${escapeHtml(o.msg)}</p>
      ${o.extra ?? ""}
      <div class="actions">
        ${o.actions}
      </div>
...
function localLoginHtml(returnTo: string, error = ""): string {
  const safeReturnTo = escapeHtml(returnTo);
  const setupUrl = `/auth/setup?returnTo=${encodeURIComponent(returnTo)}`;
  const errorDescription = error ? " auth-error" : "";
  return cardPage({
    title: "Sign in",
    heading: "Sign in to QM",
    msg: "Use the username and password your QM administrator provided.",
    icon: LOCK_ICON,
    extra: error
      ? `<p class="auth-error" id="auth-error" role="alert" aria-live="assertive" aria-atomic="true">${escapeHtml(error)}</p>`
      : "",
    actions: `<form class="auth-form" method="post" action="/auth/login" aria-labelledby="t" aria-describedby="page-description${errorDescription}">
        <input type="hidden" name="returnTo" value="${safeReturnTo}">
        <div class="auth-field">
          <label for="login-username">Username</label>
          <input id="login-username" name="username" type="text" autocomplete="username" autocapitalize="none" spellcheck="false" required autofocus>
        </div>
        <div class="auth-field">
      ...
      <a class="btn ghost" href="${escapeHtml(setupUrl)}">Set or reset a password</a>`,
    help: "Contact your QM administrator if you need an account or a one-time setup code.",
  });
}

function localSetupHtml(returnTo: string, error = ""): string {
  const safeReturnTo = escapeHtml(returnTo);
  const setupUrl = `/auth/setup?returnTo=${encodeURIComponent(returnTo)}`;
  ...
  return cardPage({
    ...
    msg: "Enter the one-time setup code from your QM administrator, then choose a new password.",
    icon: LOCK_ICON,
    extra: error
      ? `<p class="auth-error" id="auth-error" role="alert" aria-live="assertive" aria-atomic="true">${escapeHtml(error)}</p>`
      : "",
    actions: `<form class="auth-form" method="post" action="${escapeHtml(setupUrl)}" aria-labelledby="t" aria-describedby="page-description${errorDescription}">
        <input type="hidden" name="returnTo" value="${safeReturnTo}">
        <div class="auth-field">
          <label for="setup-code">One-time setup code</label>
          <input id="setup-code" name="setupCode" type="password" autocomplete="one-time-code" autocapitalize="none" spellcheck="false" required autofocus>
        </div>
        <div class="auth-field">
          <label for="setup-password">New password</label>
          <input id="setup-password" name="password" type="password" minlength="12" maxlength="256" autocomplete="new-password" aria-describedby="password-requirements" required>
        </div>
        <p class="field-help" id="password-requirements">Use 12 to 256 characters.</p>
        <div class="auth-field">
          <label for="setup-password-confirmation">Confirm new password</label>
          <input id="setup-password-confirmation" name="confirmPassword" type="password" minlength="12" maxlength="256" autocomplete="new-password" aria-describedby="password-requirements" required>
        </div>
        <button class="btn primary" type="submit">Set password and sign in</button>
      </form>
      <a class="btn ghost" href="${escapeHtml(loginUrl)}">Back to sign in</a>`,
    help: "Setup codes work once. Request a new code if yours has expired or was already used.",
  });
}
...
function sessionCookieSet(value: string, maxAge = SESSION_TTL_S): string[] {
...
const localAuthClaims = AUTH_MODE === "local" ? coreClaimStore(CORE, CORE_SIGNING_SECRET, "portal-local-auth") : null;

async function withinLocalAuthLimit(kind: string, value: string, limit: number): Promise<boolean> {
  if (!localAuthClaims) return false;
  return withinRateLimit(localAuthClaims, {
    secret: SESSION_SECRET ?? DEV_SECRET,
    kind,
    value,
    limit,
    windowS: LOCAL_AUTH_LOGIN_WINDOW_S,
    nowMs: Date.now(),
  });
}
  ...
  if (pathname === "/auth/login" && method === "GET") {
    if (AUTH_MODE === "local") {
      return sendHtml(
        res,
        200,
        localLoginHtml(sanitizeReturnTo(url.searchParams.get("returnTo"), PUBLIC_URL, APPS_DOMAIN)),
      );
    }
    return authLogin(req, res, url);
  }
  if (pathname === "/auth/login" && method === "POST" && AUTH_MODE === "local") {
    return localAuthLogin(req, res);
  }
  if (pathname === "/auth/setup" && method === "GET" && AUTH_MODE === "local") {
    return sendHtml(
      res,
      200,
      localSetupHtml(sanitizeReturnTo(url.searchParams.get("returnTo"), PUBLIC_URL, APPS_DOMAIN)),
    );
  }
  if (pathname === "/auth/setup" && method === "POST" && AUTH_MODE === "local") {
    return localAuthSetup(req, res, url);
  }
  if (pathname === "/auth/callback" && method === "GET") {
    if (AUTH_MODE === "local") return json(res, 404, { error: "not_found" });
    return authCallback(req, res, url);
  }
  if (pathname === "/auth/logout" && method === "POST") {
    if (!sameOriginRequest(req)) return json(res, 403, { error: "forbidden" });
    const localToken = AUTH_MODE === "local" ? readCookie(req.headers.cookie, "portal_session") : null;
    if (localToken) await coreLocalAuth("/logout", { token: localToken }).catch(() => undefined);
    setSession(res, [
...
async function localAuthLogin(req: IncomingMessage, res: ServerResponse): Promise<void> {
  if (!sameOriginRequest(req)) return json(res, 403, { error: "forbidden" });
  let params: URLSearchParams;
  try {
    params = new URLSearchParams(await readBody(req, 8_192));
  } catch (error) {
    if (error instanceof PayloadTooLargeError) {
      return sendHtml(res, 413, localLoginHtml("/", "The sign-in request was too large."));
    }
    throw error;
  }
  const username = (params.get("username") ?? "").trim().toLowerCase();
  const password = params.get("password") ?? "";
  const returnTo = sanitizeReturnTo(params.get("returnTo"), PUBLIC_URL, APPS_DOMAIN);
  const [userAllowed, ipAllowed] = await Promise.all([
    withinLocalAuthLimit(
      "local-login-user",
      /^[a-z0-9][a-z0-9._-]{2,63}$/.test(username) ? username : "invalid",
      LOCAL_AUTH_LOGIN_LIMIT_PER_USER,
    ),
    withinLocalAuthLimit("local-login-ip", clientIpOf(req), LOCAL_AUTH_LOGIN_LIMIT_PER_IP),
  ]);
  if (!userAllowed || !ipAllowed) {
    return sendHtml(res, 429, localLoginHtml(returnTo, "Too many sign-in attempts. Wait a few minutes and try again."));
  }
  let result: Awaited<ReturnType<typeof coreLocalAuth>>;
  try {
    result = await coreLocalAuth("/login", { username, password });
  } catch {
    return sendHtml(res, 503, localLoginHtml(returnTo, "Sign-in is temporarily unavailable."));
  }
  const token = result.body.token;
  const expiresAt = result.body.expiresAt;
  if (result.status !== 200 || typeof token !== "string" || typeof expiresAt !== "number") {
    const invalidCredentials = result.status === 401;
    return sendHtml(
      res,
      invalidCredentials ? 401 : 503,
      localLoginHtml(
        returnTo,
        invalidCredentials ? "Invalid username or password." : "Sign-in is temporarily unavailable.",
      ),
    );
  }
  setSession(res, sessionCookieSet(token, Math.max(1, Math.floor((expiresAt - Date.now()) / 1000))));
  res.writeHead(303, { location: returnTo, "cache-control": "no-store" });
  res.end();
}

async function localAuthSetup(req: IncomingMessage, res: ServerResponse, url: URL): Promise<void> {
  if (!sameOriginRequest(req)) return json(res, 403, { error: "forbidden" });
  const requestReturnTo = sanitizeReturnTo(url.searchParams.get("returnTo"), PUBLIC_URL, APPS_DOMAIN);
  let params: URLSearchParams;
  try {
    params = new URLSearchParams(await readBody(req, 8_192));
  } catch (error) {
    if (error instanceof PayloadTooLargeError) {
      return sendHtml(res, 413, localSetupHtml(requestReturnTo, "The setup request was too large."));
    }
    throw error;
  }
  const setupCode = params.get("setupCode") ?? "";
  const password = params.get("password") ?? "";
  const confirmation = params.get("confirmPassword") ?? "";
  const returnTo = sanitizeReturnTo(params.get("returnTo") ?? requestReturnTo, PUBLIC_URL, APPS_DOMAIN);
  if (password !== confirmation) return sendHtml(res, 400, localSetupHtml(returnTo, "The passwords do not match."));
  if (!(await withinLocalAuthLimit("local-setup-ip", clientIpOf(req), LOCAL_AUTH_LOGIN_LIMIT_PER_IP))) {
    return sendHtml(res, 429, localSetupHtml(returnTo, "Too many setup attempts. Wait a few minutes and try again."));
  }
  let result: Awaited<ReturnType<typeof coreLocalAuth>>;
  try {
    result = await coreLocalAuth("/setup", { setupCode, password });
  } catch {
    return sendHtml(res, 503, localSetupHtml(returnTo, "Password setup is temporarily unavailable."));
  }
  const token = result.body.token;
  const expiresAt = result.body.expiresAt;
  if (result.status !== 200 || typeof token !== "string" || typeof expiresAt !== "number") {
    const message =
      result.status === 400 && typeof result.body.message === "string"
        ? result.body.message
        : "The setup code is invalid or expired.";
    return sendHtml(res, result.status === 400 ? 400 : 503, localSetupHtml(returnTo, message));
  }
  setSession(res, sessionCookieSet(token, Math.max(1, Math.floor((expiresAt - Date.now()) / 1000))));
  res.writeHead(303, { location: returnTo, "cache-control": "no-store" });
  res.end();
...
export function bootChecks(): void {
  ...
  if (AUTH_MODE !== "oidc" && AUTH_MODE !== "local") {
    problems.push(`PORTAL_AUTH_MODE must be "oidc" or "local" (got "${AUTH_MODE}")`);
  }
  ...
  if (AUTH_MODE === "local") {
    if (PLAYGROUND) problems.push("PORTAL_AUTH_MODE=local cannot be combined with PORTAL_PLAYGROUND");
    if (AUTH_BROKER_UPSTREAM) problems.push("PORTAL_AUTH_MODE=local cannot be combined with AUTH_BROKER_UPSTREAM");
    for (const [name, value] of [
      ["PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_USER", LOCAL_AUTH_LOGIN_LIMIT_PER_USER],
      ["PORTAL_LOCAL_AUTH_LOGIN_LIMIT_PER_IP", LOCAL_AUTH_LOGIN_LIMIT_PER_IP],
    ] as const) {
      if (!Number.isInteger(value) || value < 1 || value > 64) problems.push(`${name} must be an integer from 1 to 64`);
    }
    if (
      !Number.isInteger(LOCAL_AUTH_LOGIN_WINDOW_S) ||
      LOCAL_AUTH_LOGIN_WINDOW_S < 60 ||
      LOCAL_AUTH_LOGIN_WINDOW_S > 86_400
    ) {
      problems.push("PORTAL_LOCAL_AUTH_LOGIN_WINDOW_S must be an integer from 60 to 86400");
    }
  }
  ...
  if (AUTH_MODE === "oidc" && PRINCIPAL_RULE.claim !== "sub" && PRINCIPAL_RULE.claim !== "email")
  ...
  if (
    AUTH_MODE === "oidc" &&
    (PRINCIPAL_RULE.allowedEmailDomain || PRINCIPAL_RULE.allowedEmails?.length) &&
    PRINCIPAL_RULE.claim !== "email"
  ) {
  ...
  if (IS_PROD) {
    ...
    const configuredIdentitySecret = process.env.PORTAL_IDENTITY_SECRET;
    if (isMissingOrPlaceholder(configuredIdentitySecret))
    ...
    if (configuredIdentitySecret && CORE_SIGNING_SECRET && configuredIdentitySecret === CORE_SIGNING_SECRET) {
      problems.push("PORTAL_IDENTITY_SECRET must differ from CORE_SIGNING_SECRET");
    }
    if (configuredIdentitySecret && SESSION_SECRET && configuredIdentitySecret === SESSION_SECRET) {
      problems.push("PORTAL_IDENTITY_SECRET must differ from PORTAL_SESSION_SECRET");
    }
...
export function startServer(): void {
  ...
  server.listen(PORT, () => {
    ...
    console.log(`[portal] authentication mode: ${AUTH_MODE}`);
    if (!SESSION_SECRET)
