...
async function forward(
  ...
  try {
    ...
    const forwardedIdempotencyKey =
      typeof idempotencyKey === "string" && idempotencyKey.length <= 200 ? idempotencyKey : undefined;
    const responseHeaders: Record<string, string> = {
      "content-type": "application/json",
      ...(corePath.startsWith("/v1/admin/local-users") ? { "cache-control": "no-store" } : {}),
    };
    const r = await fetch(`${CORE}${corePath}`, {
      method,
      headers: {
    ...
    for (const name of ["content-type", "content-disposition", "cache-control", "x-qm-recovery-kit-fingerprint"]) {
...
const WRITES = new Map<string, string[]>([
  ...
  ["users", ["PUT", "POST"]],
  ["local-users", ["PUT", "POST"]],
  ["slack-installation", ["PUT", "DELETE"]],
  ["matrix-installation", ["PUT", "DELETE"]],
  ["model-providers", ["PUT", "DELETE"]],
  ["backups", ["POST", "PUT", "DELETE"]],
]);
