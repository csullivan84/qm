...
test("connector setup uses the live catalog and shows exact provider and callback links", () => {
  ...
  assert.match(html, /id="card-matrix-installation"/);
  assert.match(html, /id="matrix-access-token"[^>]*type="password"|type="password"[^>]*id="matrix-access-token"/);
  assert.match(html, /id="matrix-follow-threads"/);
  assert.match(html, /id="matrix-reactions"/);
  assert.match(html, /id="matrix-attachments-enabled"/);
  assert.match(html, /id="matrix-approval-once"/);
  assert.match(html, /api\("GET", "\/api\/matrix-installation"\)/);
  assert.match(html, /api\("PUT", "\/api\/matrix-installation"/);
  assert.match(html, /api\("DELETE", "\/api\/matrix-installation"\)/);
  assert.match(html, /encrypted in durable storage/);
});

test("Recovery is a first-class Admin control plane with write-only setup and pauseable evidence refresh", () => {
  assert.match(html, /id="view-recovery"/);
  assert.match(html, /recovery: "Recovery"/);
  assert.match(
    html,
    /id="recovery-application-key"[^>]*type="password"|type="password"[^>]*id="recovery-application-key"/,
  );
  assert.match(html, /id="recovery-operational-recipient"/);
  assert.match(html, /id="recovery-lock-required"/);
  assert.match(html, /id="recovery-pause-refresh"/);
  assert.match(html, /api\("GET", "\/api\/backups\/status"\)/);
  assert.match(html, /api\("PUT", "\/api\/backups\/config"/);
  assert.match(html, /\/api\/backups\/recovery-kit/);
  assert.match(html, /\/api\/backups\/recovery-points\/.*\/restore-drill/);
  assert.match(html, /document\.hidden \|\| \$\("recovery-pause-refresh"\)\.checked/);
  assert.match(html, /recoveryConfigDirty && !editing|!recoveryConfigDirty && !editing/);
  assert.match(html, /role="status" aria-live="polite"/);
  assert.doesNotMatch(html, /\$\("recovery-application-key"\)\.textContent/);
});
