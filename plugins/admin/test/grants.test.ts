...
test("local account creation and password reset actions are source-authenticated and keep their bodies scoped", async () => {
  const create = await fetch(`${base}/api/local-users`, {
    method: "POST",
    headers: { cookie: ADMIN, "content-type": "application/json" },
  ...
  for (const [method, path, body] of [
    ["PUT", "/api/local-users/new-user/password", { password: "fixture-password-value" }],
    ["POST", "/api/local-users/new-user/password-reset", {}],
    ["PUT", "/api/local-users/new-user/enabled", { enabled: false }],
  ] as const) {
    ...
    assert.equal(call.url, `/v1/admin${path.slice(4)}`);
...
test("grants/users require a signed-in cookie → 401 when absent (no core hop)", async () => {
  ...
  assert.equal(
    (
      await fetch(`${base}/api/local-users`, {
        method: "POST",
