...
test(
  ...
  async () => {
    ...
    try {
      ...
      ) as Record<string, { filename: string; files: Array<{ path: string }> }>;
      const artifact = packed["@yc-software/qm"]!;
      const tarball = join(dir, artifact.filename);
...
`,
      ...
      assert.ok(artifact.files.some(({ path }) => path === "LICENSE"));
      assert.ok(artifact.files.some(({ path }) => path === "templates/aws/microvm-agent/Dockerfile"));
      assert.ok(artifact.files.some(({ path }) => path === "templates/aws/microvm-agent/agent.mjs"));
      assert.ok(artifact.files.some(({ path }) => path === "templates/deployment/deployment.md"));
      assert.ok(artifact.files.some(({ path }) => path === "templates/deployment/references/fly.md"));
      assert.ok(artifact.files.some(({ path }) => path === "templates/fly/core.toml"));
      assert.ok(!artifact.files.some(({ path }) => path === "src/contract.ts" || path === "bin/qm.ts"));
    } finally {
