...
test("nodeEngineProblem accepts a satisfying version and rejects an older one", () => {
  assert.equal(nodeEngineProblem(">=26.5.1", "package.json", "v26.5.1"), undefined);
  assert.equal(nodeEngineProblem(">=24.0.0", "package.json", "v24.13.0"), undefined);
  const problem = nodeEngineProblem(">=26.5.1", "the repo package.json", "v26.5.0");
  assert.ok(problem?.includes("v26.5.0"));
  assert.ok(problem?.includes(">=26.5.1"));
