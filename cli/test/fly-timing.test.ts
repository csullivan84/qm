...
test("a fresh local-auth Fly deploy provisions and stages Managed Postgres before deploying", () => {
  const dir = mkdtempSync(join(tmpdir(), "qm-fly-fresh-pg-"));
  const logPath = join(dir, "fly.log");
  const configPath = join(dir, "local-auth.json");
  writeFileSync(
    configPath,
    JSON.stringify({
      contract: 1,
      ...
      env: {
        ...
        portal: { PORTAL_AUTH_MODE: "local" },
      },
    }),
  );
  execFileSync(process.execPath, [bin, "up", "--config", configPath, "--only", "core", "--image-from", "source"], {
    encoding: "utf8",
    cwd: repoRoot,
    env: {
      ...process.env,
