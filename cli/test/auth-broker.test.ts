...
test("local database auth needs no OIDC trust configuration and cannot also run the email broker", () => {
  const local = configWith(
    configText({
      services: '["core", "web-ui", "admin", "portal"]',
      env: '{ "core": { "LOCAL_AUTH_BOOTSTRAP_USER": "alice" }, "portal": { "PORTAL_AUTH_MODE": "local" } }',
    }),
  );
  assert.doesNotThrow(() =>
    validatePortalTrust(
      local,
      "config",
      new Map([["LOCAL_AUTH_BOOTSTRAP_TOKEN", "fixture-bootstrap-token-that-is-long-enough"]]),
    ),
  );
  assert.throws(() => validatePortalTrust(local, "config", new Map()), /LOCAL_AUTH_BOOTSTRAP_TOKEN/);
  assert.throws(
    () =>
      configWith(
        configText({
          env: '{ "auth": { "AUTH_EMAIL_TRANSPORT": "resend", "AUTH_ALLOWED_EMAIL_DOMAIN": "example.com" }, "portal": { "PORTAL_AUTH_MODE": "local" } }',
        }),
      ),
    /cannot be combined with the "auth" service/,
  );
  assert.throws(
    () =>
      configWith(
        configText({
          services: '["core", "admin", "portal"]',
          env: '{ "core": { "LOCAL_AUTH_BOOTSTRAP_USER": "Alice" }, "portal": { "PORTAL_AUTH_MODE": "local" } }',
        }),
      ),
    /LOCAL_AUTH_BOOTSTRAP_USER/,
  );
  assert.throws(
    () =>
      configWith(
        configText({
          ...
          env: '{ "portal": { "PORTAL_AUTH_MODE": "local" } }',
        }),
      ),
    /requires the "admin" service/,
  );
});
