...
export function validatePortalTrust(config: QmConfig, path = "config", secrets?: ReadonlyMap<string, string>): void {
  ...
  const authMode = config.env.portal?.PORTAL_AUTH_MODE?.trim() || "oidc";
  if (authMode !== "oidc" && authMode !== "local") {
    throw new CliError(`${path}: env.portal.PORTAL_AUTH_MODE must be "oidc" or "local"`);
  }
  if (authMode === "local") {
    if (config.services.includes("auth")) {
      throw new CliError(`${path}: env.portal.PORTAL_AUTH_MODE=local cannot be combined with the "auth" service`);
    }
    if (!config.services.includes("admin")) {
      throw new CliError(
        `${path}: env.portal.PORTAL_AUTH_MODE=local requires the "admin" service for account management`,
      );
    }
    const bootstrapUser = config.env.core?.LOCAL_AUTH_BOOTSTRAP_USER?.trim();
    if (bootstrapUser !== undefined && !/^[a-z0-9][a-z0-9._-]{2,63}$/.test(bootstrapUser)) {
      throw new CliError(
        `${path}: env.core.LOCAL_AUTH_BOOTSTRAP_USER must be 3 to 64 lowercase letters, numbers, dots, underscores, or hyphens`,
      );
    }
    if (secrets && bootstrapUser) {
      const setupCode = secrets.get("LOCAL_AUTH_BOOTSTRAP_TOKEN")?.trim();
      if (isMissingOrPlaceholder(setupCode) || setupCode!.length < 32) {
        throw new CliError(
          `${path}: LOCAL_AUTH_BOOTSTRAP_TOKEN must be a non-placeholder value of at least 32 characters when LOCAL_AUTH_BOOTSTRAP_USER is configured`,
        );
      }
    }
    return;
