...
export const FIRST_PARTY_SECRET_SPECS: readonly SecretSpec[] = [
  ...
  {
    name: "LOCAL_AUTH_BOOTSTRAP_TOKEN",
    service: "core",
    required: { when: { kind: "env-present", service: "core", name: "LOCAL_AUTH_BOOTSTRAP_USER" } },
    description:
      "One-time setup code for the initial local administrator; remove it and LOCAL_AUTH_BOOTSTRAP_USER after that account is configured.",
    generate: MINT_LOCALLY,
  },
  {
      ...
      when: {
        ...
        conditions: [
          { kind: "service-absent", service: "auth" },
          {
            kind: "any",
            conditions: [
              { kind: "env-absent", service: "portal", name: "PORTAL_AUTH_MODE" },
              { kind: "env-equals", service: "portal", name: "PORTAL_AUTH_MODE", value: "oidc" },
            ],
          },
