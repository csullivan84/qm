...
const POSTGRES_IMAGE = "postgres:18.4@sha256:3a82e1f56c8f0f5616a11103ac3d47e632c3938698946a7ad26da0df1334744a";
...
function postgresMajorInContainer(name: string): number | undefined {
  if (!containerRunning(name)) return undefined;
  const output = docker(["exec", name, "postgres", "--version"]);
  const match = /PostgreSQL\) (\d+)/.exec(output);
  return match ? Number(match[1]) : undefined;
}

function postgresMajorInVolume(ctx: DockerCtx): number | undefined {
  if (!volumeExists(pgVolume(ctx))) return undefined;
  const output = docker([
    "run",
    "--rm",
    "-v",
    `${pgVolume(ctx)}:/inspect:ro`,
    POSTGRES_IMAGE,
    "sh",
    "-c",
    "if [ -f /inspect/PG_VERSION ]; then cat /inspect/PG_VERSION; elif [ -f /inspect/18/docker/PG_VERSION ]; then cat /inspect/18/docker/PG_VERSION; fi",
  ]).trim();
  return /^\d+$/.test(output) ? Number(output) : undefined;
}

function requirePostgres18(ctx: DockerCtx): void {
  const containerMajor = postgresMajorInContainer(cname(ctx, "pg"));
  const volumeMajor = postgresMajorInVolume(ctx);
  const incompatible = containerMajor ?? volumeMajor;
  if (incompatible !== undefined && incompatible !== 18) {
    throw new CliError(
      `Postgres ${incompatible} data exists for ${ctx.config.orgId}. ` +
        "QM refuses an in-place major-version start: create a PostgreSQL 18 target and migrate with pg_dump/pg_restore.",
    );
  }
}
    ...
    if (!containerRunning(pgName)) {
      ...
      try {
        docker([
          ...
          "POSTGRES_DB=qm",
          "-v",
          `${pgVolume(ctx)}:/var/lib/postgresql`,
          POSTGRES_IMAGE,
        ]);
      } finally {
        secretFile.cleanup();
