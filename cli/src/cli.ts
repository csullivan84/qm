...
import { runBackupCommand } from "./commands/backup.ts";
...
${bold("DEPLOY (operator)")} ${dim("— runs in the deployment directory")}
  ...
  backup kit inspect --kit-file <path>     inspect a passphrase-encrypted offline recovery kit
  backup list --kit-file <path>            list B2 recovery points without the core
  backup inspect <point> --kit-file <path> inspect a recovery-point manifest
  backup verify <point> --kit-file <path>  download, checksum, and decrypt-test a recovery point
  backup restore <point> --kit-file <path> restore into an explicit qm_restore_* PostgreSQL target
     --target-database-url-file <path> --output-dir <path>
  backup prepare-cutover <restored-dir> --kit-file <path>
                                           print the approval-gated cutover checklist
...
async function dispatch(argv: string[]): Promise<void> {
  ...
  switch (cmd) {
    ...
    case "backup": {
      return runBackupCommand(positionals, flags);
    }
