...
The downstream keeps the inherited package name so existing deployment-directory imports
remain compatible, but this repository does not publish that npm scope or any parent
project container namespace. `cli/package.json` is marked private and the inherited
release workflows are intentionally absent. Pull parent updates through the read-only
upstream relationship, then review and release any downstream artifacts only under
downstream-owned names. The checked-in image manifest is a sentinel that a deployment
overrides with real digests. The packed-artifact test exercises the consumer path locally.
